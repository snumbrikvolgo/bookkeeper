...
poetry run flake8 bookkeeper --max-complexity 10
...
1. Сделать fork репозитория и склонировать его себе на компьютер - [x]
2. Написать класс SqliteRepository - [x]
3. Написать тесты к этому классу - [x]
4. Подключить СУБД sqlite к simple_client (пока он работает в оперативной памяти и все забывает при выходе) - [x]
5. Простая модель бюджета* - [x]

Задача второго этапа:
1. Создать виджеты:
   - для отображения списка расходов с возможностью редактирования - [x]
   - для отображения бюджета на день/неделю/месяц с возможностью редактирования - [x]
   - для добавления нового расхода - [x]
   - для просмотра и редактирования списка категорий - [x]
2. Собрать виджеты в главное окно - [x]
